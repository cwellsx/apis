This contains the scipt which is loaded into the webview -- therefore:
...
- I tried using `include` in the tsconfig.json ...
  that worked for `compile.cmd` but not for Intellisence
...
is to specify this "TypeScript triple-slash directorive" in the script.ts:

```ts
/// <reference path="./ipc.d.ts" />
```

Anaother difference between this script and the others is that
