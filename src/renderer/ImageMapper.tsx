...
export const ImageMapper: FC<ImageMapperProps> = ({
  ...
  const drawcircle = (coords: number[], fillColor: string, lineWidth: number, strokeColor: string) => {
    const ctx = canvas.current?.getContext("2d");
    if (!ctx) return;
    ctx.fillStyle = fillColor;
    ctx.beginPath();
  ...
  const drawpoly = (coords: number[], fillColor: string, lineWidth: number, strokeColor: string) => {
    const coords1: number[][] = coords.reduce((a, _, i, s) => (i % 2 ? a : [...a, s.slice(i, i + 2)]) as any, []);
    const ctx = canvas.current?.getContext("2d");
    if (!ctx) return;
    ctx.fillStyle = fillColor;
    ctx.beginPath();
  ...
  const imageClick = (event: React.MouseEvent<HTMLImageElement, MouseEvent>) => {
    if (onImageClick) {
      event.preventDefault();
      onImageClick(event);
    }
  ...
  const imageMouseMove = (event: React.MouseEvent<HTMLImageElement, MouseEvent>) => {
    if (onImageMouseMove) {
